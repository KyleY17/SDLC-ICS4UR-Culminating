 ...
 * Mage tower.
 *
 * Shoots invisible chain-starter projectiles that later
 * generate a chain of lightning hits and visual bolts.
 *
 * Key behavior:
 * - Damage scales with level.
 * - Chain length and chaining range grow with level.
 */
public class MageTower extends Tower {
    ...
    public MageTower(double x, double y) {
        super(x, y, 38, 140, 10, 90, 250, 160,
    ...
    // Level scaling (Level 2-5): modify damage, chain counts/range, and fire cadence.

    @Override
    protected void applyUpgrade() {
        switch (level) {
            // Level 2: more bounces and range
            case 2 -> { damage *= 1.3; chainBounces = 3; chainRange += 20; }
            // Level 3: even more bounces and faster firing
            case 3 -> { damage *= 1.5; chainBounces = 4; chainRange += 20; fireRate = Math.max(50, fireRate - 15); }
            // Level 4: lots of bounces, very powerful
            case 4 -> { damage *= 1.5; chainBounces = 5; chainRange += 30; fireRate = Math.max(30, fireRate - 20); chainFalloff = 0.75; }
            // Level 5 (MAXED): INSANE bounces, lightning everywhere
            case 5 -> { damage *= 2.0; chainBounces = 7; chainRange += 40; fireRate = Math.max(18, fireRate - 12); chainFalloff = 0.85; }
        }
    }
    ...
    public Projectile createProjectile() {
        ...
        return new Projectile(m.x, m.y, currentTarget,
                              damage, 9,
                              getLightningColor(), 8,
                              false, 0, false, 1.0, 0).setChainStarter(this).setInvisible();
    }

    /**
     * Selects the next target to bounce to.
     *
     * Strategy:
     * - Consider only enemies not already hit and still active.
     * - Ignore candidates beyond chainRange (plus a small buffer).
     * - Choose the closest valid enemy.
     */
    ...
    /**
     * Applies chain lightning damage and creates visual lightning bolts.
     *
     * allEnemies full enemy list (used to find bounce targets)
     * startEnemy first enemy to take full damage
     * damageFunc callback that applies damage to an enemy
     * list of objects representing the bolt visuals
     */
    public List<Projectile> applyChainDamage(List<Enemy> allEnemies, Enemy startEnemy,
        ...
        List<Enemy> chainPath = new ArrayList<>();  // Track the path of all hit enemies
        Set<Enemy> hit = new HashSet<>();
        
        if (startEnemy == null) return visuals;
        hit.add(startEnemy);
        chainPath.add(startEnemy);
        
        // Fork depth determines how many levels deep the lightning can fork
        int forkDepth = Math.min(chainBounces, 4);
        
        // Apply damage to first enemy and start recursive forking
        double dmg = damage;
        damageFunc.accept(startEnemy, dmg);
        
        // Fork lightning and track the path
        forkLightning(allEnemies, startEnemy, hit, chainPath, dmg, 0, forkDepth, visuals, damageFunc);
        
        // Create a single continuous line through the chain path (no tower connection)
        for (int i = 0; i < chainPath.size() - 1; i++) {
            Enemy from = chainPath.get(i);
            Enemy to = chainPath.get(i + 1);
            visuals.add(Projectile.visualChain(
                from.getCenterX(), from.getCenterY(),
    ...
    /**
     * Recursive bounce/fork logic.
     *
     * Each call represents the lightning at currentEnemy and can branch to up to
     * maxForksPerNode new targets, up to maxDepth levels.
     */
    private void forkLightning(List<Enemy> allEnemies, Enemy currentEnemy, Set<Enemy> hit,

                               List<Enemy> chainPath, double damage, int currentDepth, int maxDepth,
                               List<Projectile> visuals, java.util.function.BiConsumer<Enemy, Double> damageFunc) {
        // Stop if we've reached max depth
        if (currentDepth >= maxDepth) return;
        
        // Each fork can split into up to 2 targets
        int maxForksPerNode = 2;
        
        for (int fork = 0; fork < maxForksPerNode; fork++) {
            Enemy target = findNextTarget(allEnemies, currentEnemy, hit);
            if (target == null) break;
            
            hit.add(target);
            chainPath.add(target);  // Add to the path in discovery order
            
            // Damage decreases with each bounce
            double nextDamage = damage * chainFalloff;
            
            // Apply damage
            damageFunc.accept(target, nextDamage);
            
            // Recursively fork from this new target
            forkLightning(allEnemies, target, hit, chainPath, nextDamage, currentDepth + 1, maxDepth,
                         visuals, damageFunc);
        }
    }

    /**
     * Lightning color changes with tower level.
     */
