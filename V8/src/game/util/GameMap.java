...
/**
 * This defines the game map - the layout of the terrain, the path enemies walk on, and obstacles.
 */
public class GameMap {
    // Each tile is 48 pixels
    public static final int TILE   = 48;
    // The map is 18 tiles wide
    public static final int COLS   = 18;
    // The map is 13 tiles tall
    public static final int ROWS   = 13;
    // Total width: 18 * 48 = 864 pixels
    public static final int WIDTH  = COLS * TILE;
    // Total height: 13 * 48 = 624 pixels
    public static final int HEIGHT = ROWS * TILE;

    // Which tiles are part of the path?
    private final boolean[][] pathGrid = new boolean[ROWS][COLS];
    // Which tiles have towers on them?
    private final boolean[][] occupied = new boolean[ROWS][COLS];
    // Which tiles have trees on them?
    private final boolean[][] treeTile = new boolean[ROWS][COLS];

    // The list of points that make up the enemy path
    private final List<Point2D.Double> waypoints = new ArrayList<>();

    // Fixed tree positions
    // Each row is {column, row} coordinates
    ...
    // Grass colors (checkerboard pattern)
    ...
    // Create the game map
    public GameMap() {
        // Build the path that enemies walk on
        buildPath();
        // Place the trees
        markTrees();
    }
