...
/**
 * Cannon tower implementation.
 *
 * Responsibilities:
 * - Levels up by scaling its damage/range/fire rate.
 * - Shoots splash projectiles,
 * - Renders its own visual style.
 */
public class CannonTower extends Tower {
    ...
    // Apply stat changes when the tower levels up (Level 2-5).
    @Override
    protected void applyUpgrade() {
        switch (level) {
            ...
            // Level 4: 
            case 4 -> { damage *= 1.5; range += 20; fireRate = Math.max(10, fireRate - 5); }
            // Level 5 (MAXED): 
            case 5 -> { damage *= 1.5; range += 15; fireRate = Math.max(6, fireRate - 4); }
        }
    }

    // Create the projectile that this tower shoots
    @Override
    public Projectile createProjectile() {
        var muzzle = getMuzzlePoint();
        // Higher levels shoot bigger, more powerful projectiles
