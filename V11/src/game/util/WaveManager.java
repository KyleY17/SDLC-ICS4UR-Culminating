...
public class WaveManager {
    ...
    private void buildWave(int wave) {
        ...
        if (wave <= 15) {
            ...
            add(EnemyType.BASIC, basicCount, Math.max(15,30-extra), hp * (0.5*extra), spd);
            add(EnemyType.FAST,  fastCount,  Math.max(12,25-extra), hp * (0.5*extra), spd);
            add(EnemyType.TANK,  tankCount,  Math.max(25,45-extra), hp * (0.5*extra), spd);
            add(EnemyType.BOSS,  bossCount,  Math.max(60,80-extra), hp * (0.5*extra), spd);
        }
