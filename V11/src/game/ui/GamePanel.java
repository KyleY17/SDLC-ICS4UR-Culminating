...
public class GamePanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    public GamePanel(SidePanel sidePanel, Difficulty difficulty) {
        ...
        this.lives = difficulty == Difficulty.EASY ?  25 : difficulty == Difficulty.HARD ?  2000 :  20;
        this.score = 0;
    ...
    // Chain fires whether or not the primary enemy survived the hit
    if (primary != null && p.isChainStarter() && p.getTower() instanceof game.entities.towers.MageTower mage) {
    ...
    if (p.isSplash()) {
        ...
        for (Enemy en : enemies) {
            ...
            if (Math.sqrt(dx*dx + dy*dy) <= p.getSplashRadius()) {
                applyDamage(en, p.getDamage() * 0.5, p.isSlow(), p.getSlowMultiplier(), p.getSlowDuration());
            }
        }
    }
}
    ...
    private void applyDamage(Enemy en, double dmg, boolean slow, double slowMult, int slowDur) {
        en.takeDamage(dmg);
        if (slow && en.getFreezeCount() < 3) en.applySlow(slowMult, slowDur);
        if (!en.isActive()) {
    ...
    private void sellSelected() {
        ...
        int refund = (int)Math.round(selectedTower.getMoneySpent()*0.8f);
        gold += refund;
