...
public class SidePanel extends JPanel {
    ...
    // Current cost multiplier from difficulty
    private double costMult=1.0;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (placingType != null) {
            ...
            g2.drawString(hint1, w / 2 - g2.getFontMetrics().stringWidth(hint1) / 2, 574);
            g2.drawString(hint2, w / 2 - g2.getFontMetrics().stringWidth(hint2) / 2, 587);
        }
    }
    ...
    public void updateStats(int gold, int lives, int score, int wave, boolean waveInProgress,
        ...
        for (int i = 0; i < towerBtns.length; i++) {
            int baseCost = TowerFactory.getCost(towerTypes[i]);
            int actualCost = (int)(baseCost * costMult);
            ...
            towerBtns[i].setBackground(canAfford
                ...
                : new Color(55, 38, 18));
            // Update button text with difficulty-adjusted price
            towerBtns[i].setText(
                "<html><center><b>" + TowerFactory.getName(towerTypes[i]) + "</b><br>"
                + "<font size='2'>" + descs[i] + " — $" + actualCost + "</font></center></html>");
        }
        repaint();
    }
